...
def sigmoid_accuracy(preds, labels):
    ...
    correct_prediction = tf.equal(tf.cast(preds >= 0.0, tf.int64),
...
def binary_accuracy(preds, labels):
    ...
    :return: average accuracy
    """
    correct_prediction = tf.equal(tf.cast(preds >= 0.5, tf.int64),
                                  tf.to_int64(labels))
    accuracy_all = tf.cast(correct_prediction, tf.float32)
    return tf.reduce_mean(accuracy_all)
...
def binary_cross_entropy(outputs, labels):
    ...
    cross_entropy = tf.reduce_mean(labels * -tf.log(outputs) +
                                   (1 - labels) * -tf.log(1 - outputs))
